...
class Template(nn.Module):
    ...
    def create_new_templates(self, var: int) -> None:
        ...
        p_T = [alpha / n_square for _ in range(int(((self.out_size // self.stride)**2)))]
...
class ActionPredictor(nn.Module):    
    def __init__(self, 
        ...
        self.preprocess = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=8, stride=2, padding=4),
            nn.SiLU(),
            nn.Conv2d(16, 32, kernel_size=5, stride=2, padding=0),
            nn.SiLU(),
            nn.Conv2d(32, 32, kernel_size=4, stride=2, padding=0),
            nn.SiLU(),
            SqueezeAggregateLayer(32, args.n_template, aggregate='sum')
        )

        with torch.no_grad():
            self.template_size = self.preprocess(torch.rand((1, *input_dim))).shape[-1]
        self.template_counts = args.n_templates
        self.template = torch.jit.script(Template(M=args.n_template, out_size=self.template_size, var=args.template_var, stride=2, device=args.device))
        
        self.backbone = nn.Sequential(
            nn.Conv2d(args.n_template, 32, kernel_size=4, stride=2, padding=2),
            nn.SiLU(),
        ...
        with torch.no_grad():
            self.cnn_output_size = self.backbone(torch.rand((1, args.n_templates, self.template_size, self.template_size))).shape[-1]
...
class RNDModel(nn.Module):
    def __init__(self, input_size, output_size, action_size):
        ...
        self.masker = ActionPredictor(self.input_size, action_size)
    ...
    def forward(self, next_obs):
        mask = self.masker.get_mask(next_obs)
        mask = F.upsample_nearest(mask, size=self.output_size)
